...
class BasicModelTrain:
    def __init__(self, batch_size, n_feat, vocab_size, embedding_size, max_encoder_time, max_decoder_time,
                 hidden_units, max_gradient_norm=100, learning_rate=0.0001):
        ...
        with tf.variable_scope("encoder"):
            self.encoder_cell = tf.nn.rnn_cell.BasicRNNCell(hidden_units)
        ...
        with tf.variable_scope("decoder"):
            ...
            self.decoder_cell = tf.nn.rnn_cell.BasicRNNCell(hidden_units)
            self.decoder_inputs_embedded = tf.nn.embedding_lookup(self.embeddings, self.decoder_inputs)
            print(self.decoder_inputs_embedded.shape)
            ...
            self.logits = tf.nn.softmax(self.outputs.rnn_output)

...
class AttentionModel_Train:
    ...
    def __init__(self, batch_size, n_feat, vocab_size, embedding_size, max_encoder_time, max_decoder_time,
        ...
        with tf.variable_scope("decoder"):
            ...
            self.decoder_initial_state = self.decoder_cell.zero_state(batch_size, tf.float32).clone(
                cell_state=self.encoder_final_state)

            # batch_size & max_decoder_time error
            self.decoder = tf_seq2seq.BasicDecoder(
                cell=self.decoder_cell, helper=self.helper, initial_state=self.decoder_initial_state,
...
class AttentionModel_Infer:
    def __init__(self, batch_size, n_feat, vocab_size, embedding_size, max_encoder_time, max_decoder_time,
        ...
        with tf.variable_scope("decoder"):
            ...
            self.attention_mechanism = tf_seq2seq.LuongAttention(hidden_units, self.attention_states)
            self.decoder_cell = tf.contrib.seq2seq.AttentionWrapper(
                self.decoder_cell, self.attention_mechanism,
                attention_layer_size=hidden_units)

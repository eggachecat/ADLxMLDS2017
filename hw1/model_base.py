...
class Edit_Distance_Callback(keras.callbacks.Callback):
    def __init__(self, x_seq, y_seq, data_dir="./data/", decay_rate=None, n_epoch=10, exp_name="233"):
        self.x_seq = x_seq
        self.y_seq = y_seq
        self.idx_phone_map, self.phone_char_map = get_idx_phone_map(data_dir)
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        true_phone_list = []
        for seq in y_seq:
            true_phone_list.append("".join(trim([self.phone_char_map[idx_phone_map[i]] for i in seq])))
        ...
        for seq in x_seq:
            ...
            phone_seq = trim([self.phone_char_map[idx_phone_map[i]] for i in prediction])
        ...
        with open("./{ep}_history.txt".format(ep=self.exp_name), "w") as output:
            output.write("\n".join(self.history))


class Sequence_Edit_Distance_Callback(keras.callbacks.Callback):
    def __init__(self, x_seq, y_seq, data_dir="./data/", max_len=777, exp_name="233", decay_rate=None, n_epoch=10):
        self.x_seq = x_seq
        self.y_seq = y_seq
...
class HW1Model:
    ...
    def to_one_hot(_labels):
        labels = np.array(list(range(N_CLASS)), dtype=int)
        enc = OneHotEncoder()
    ...
    @staticmethod
    def seq_predict(model, test_seq, target_dir, data_dir="./data/", max_len=777):
        ...
        pred_phone_list = []
        idx_phone_map, phone_char_map = get_idx_phone_map(data_dir)
    ...
    @staticmethod
    def seq_predict_matrix(model, test_seq, target_dir, data_dir="./data/", max_len=777):
    ...
    @staticmethod
    def multi_seq_predict(model_list, test_seq, target_dir, data_dtr="./data/", max_len=777):
        ...
        for i, model in enumerate(model_list):
            print("Predicting model {n} ...".format(n=i))
            if y_pred is None:
                y_pred = model.predict(x_pred, verbose=1, batch_size=64)
            else:
                y_pred += model.predict(x_pred, verbose=1, batch_size=64)
            print("Done")
        prediction = []
        ...
        pred_phone_list = []
        idx_phone_map, phone_char_map = get_idx_phone_map(data_dtr)
        ...
        for seq in prediction:
            _seq = np.argmax(seq, axis=1)
            rtn_prediction.append(_seq)
            __seq = "".join(trim([phone_char_map[idx_phone_map[i]] for i in _seq], use_super_trim=8))
            ctr += 1
            pred_phone_list.append(__seq)
    ...
    @staticmethod
    def train(model, x_train, y_train, x_valid, y_valid, batch_size, exp_name, max_len=777, callback=None,
              data_src="mfcc",
              sample_weight=None):

        model_dir = "./outputs/models/model_{name}_{en}".format(name=data_src, en=exp_name)
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)

        log_dir = "./outputs/logs/log_{name}_{en}".format(name=data_src, en=exp_name)

...
class HW1Model:
    ...
    @staticmethod
    def load_model(model_path, weight_name):
        model = keras.models.load_model(model_path + '/basic_model.h5')
        model.load_weights(model_path + "/weights." + weight_name + ".hdf5")
        return model

    @staticmethod
    def predict(model, x_data, index, model_type, exp_name, seq=True, max_len=777):
        ...
        res.to_csv("./outputs/results/{mt}/{en}.csv".format(mt=model_type, en=exp_name), header=True, index=False)

...
def get_train_data_sequence():
    ...
    try:
        data_table_x = np.load("./data/train_x.npy")
        data_table_y = np.load("./data/train_y.npy")
    except:
        _df = pd.read_hdf("./data/train_mfcc.hdf5", 'train')
        ...
        np.save("./data/train_x.npy", data_table_x)
        np.save("./data/train_y.npy", data_table_y)
    print("66666")
...
def validate(exp_name, check_point_name):
    ...
    model = keras.models.load_model("./outputs/models/mlp/model_" + exp_name + '/basic_model.h5')
    model.load_weights("./outputs/models/mlp/model_" + exp_name + "/weights." + check_point_name + ".hdf5")
...
def validate_seq(exp_name, check_point_name):
    ...
    x_valid, y_valid = get_train_data_sequence()
    ...
    for seq in x_valid:
        ...
        phone_seq = trim([phone_char_map[int_phone_map[i]] for i in prediction])
...
def predict(exp_name, check_point_name):
    ...
    res = pd.DataFrame({'id': index, 'phone_sequence': phone_list})
    res.to_csv("./outputs/results/lstm/{f}.csv".format(f=exp_name), header=None, index=False)


# train()
# validate_seq("1508021128.0596418", "03-1.48401")
